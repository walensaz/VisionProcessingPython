...
starting_1 = np.asarray([0, 0, 75])  # white!
starting_2 = np.asarray([5, 5, 95])  # yellow! note the order

color_1 = np.asarray([0, 0, 75])  # white!
color_2 = np.asarray([5, 5, 95])  # yellow! note the order


region_tab = (0, 0, 1900, 50)

region_game = (20, 40, 1900, 900)
def main():
    inWork = False
    while True:
        if inWork:
            img = custom_get_positions(region=region_tab)
            newimg = U.draw_custom_contours(color_1, color_2, img)
            newimg = cv2.resize(newimg, (1300, 700))
            cv2.imshow('window', newimg)
        else:
            img = custom_get_positions(region=region_game)
            newimg = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            newimg = cv2.resize(newimg, (1500, 900))
            cv2.imshow('window', newimg)
